    ...
    getters: {
        ...
        categorySize: (state) => {
            ...
            return Object.fromEntries(
                Object.entries(state.hypernetworks)
                    .filter(
                        ([, v]) =>
                            settings.showRestricted ||
                            v.content.some((e) => !e.restricted)
        ...
        allHypernetworks: (state) => {
            ...
            return ImmutableSet<Hypernetwork>(
                Object.values(state.hypernetworks).reduce(
                    (a: Hypernetwork[], b) => {
                        return a.concat(
                            b.content.filter(
    ...
    actions: {
        async load() {
            ...
            const hnData: Hypernetworks = {
                hypernetworks: result.reduce(
                    (a: HypernetworkCategories, p: HypernetworkFile) => {
        ...
        resolve(name: string) {
            const meta = this.allHypernetworks.find(
        ...
        searchCategory(category: string, query: string): Hypernetwork[] {
            const settings = useSettingsStore()
            if (query === '')
                return this.hypernetworks[category].content.filter(
                    (n) => settings.showRestricted || !n.restricted
                )
            ...
            >(
                ...
                this.hypernetworks[category].content.map((n) => ({
                    ...n,
                    score: 0,
                }))
            )
